...
class LocalViKeyPointBottleneck(KeyPointBottleneck):
    ...
    def __init__(self, _key_point_num, _gauss_std, _output_dim, _flatten='conv2d'):
        super().__init__(_key_point_num, _gauss_std, _output_dim, _flatten=_flatten)
        if _flatten == 'conv2d':
            self.key_point_heatmap_encoder = KeyPointHeatmapEncoder(layer_params={'filter num': [16, 32],
                                                                                  'operator': ['conv2d', 'conv2d'],
                                                                                  'kernel sizes': [3, 3],
                                                                                  'strides': [1, 1]}
                                                                    )
        self.to_hidden_node = nn.Sequential(nn.Linear(144, _output_dim),  # 1458
                                            nn.ReLU()
                                            )
    ...
    def forward(self, k_heat_maps, vi_mode_on=True):
        ...
        :return: N * k * h * w heatmaps by visual keypoint bottleneck.
        """
        gauss_mu = self._soft_max_key_points(k_heat_maps)*5  # centers
        map_size = k_heat_maps.shape[2:4]
        gauss_maps = self._get_gaussian_maps(gauss_mu, map_size)  # gauss normalized heatmaps
        if vi_mode_on:
            # print('vi_mode ON')
        ...
        N, k = vi_key_point_heatmaps.shape[0:2]
        # crop it.
        centered_heatmaps = self._get_centered_key_point_heatmaps(vi_key_point_heatmaps, gauss_mu, map_size)
        ...
        # print(h.shape)
        h = self.key_point_heatmap_encoder(centered_heatmaps)  # N*k * output dim
        # print(h.shape)
        h = self.to_hidden_node(h.reshape(N, k, -1))  # # N * k * output dim
        # print(h.shape)
        return h, gauss_mu, gauss_maps, centered_heatmaps

    def _get_centered_key_point_heatmaps(self, heatmap, mu, map_size, crop_size=[16, 16]):
        """Transforms the keypoint center points to a gaussian masks."""
        """heatmap: N * C * H * W"""
        # empty heatmap with cropsize
        centered_heatmap = torch.zeros(heatmap.shape[0], heatmap.shape[1], crop_size[0], crop_size[1]).to(heatmap.device)
        # N * k * hy * wx
        mu_x, mu_y = mu[:, :, 0:1], mu[:, :, 1:2]
        ...
        # y = torch.linspace(-1.0, 1.0, map_size[0]).type(torch.FloatTensor)
        # locate cx, cy
        cx = torch.round((mu_x + 1)*(map_size[1]-1)/2)
        cy = torch.round((mu_y + 1) * (map_size[0] - 1) / 2)
        # print('cx {}, cy {}'.format(cx[0,3], cy[0,3]))

        for i in range(heatmap.shape[0]):  # N
            for j in range(heatmap.shape[1]):  # K maps
                cx_left = int(cx[i, j].item() - int(crop_size[1]/2)) if cx[i, j].item() - int(crop_size[1]/2)>0 else 0
                ccx_left = int(int(crop_size[1]/2) - cx[i, j].item()) if cx[i, j].item() - int(crop_size[1]/2)<0 else 0
                cx_right = int(cx[i, j].item() + int(crop_size[1]/2)) if cx[i, j].item() + int(crop_size[1]/2)<map_size[1]-1 else map_size[1]

                cy_left = int(cy[i, j].item() - int(crop_size[0] / 2)) if cy[i, j].item() - int(crop_size[0] / 2) > 0 else 0
                ccy_left = int(int(crop_size[0]/2) - cy[i, j].item()) if cy[i, j].item() - int(crop_size[0]/2) <0 else 0
                cy_right = int(cy[i, j].item() + int(crop_size[0] / 2)) if cy[i, j].item() + int(crop_size[0] / 2) < map_size[0] else map_size[0]
