...
from examples.rl_sac.agent import Agent
...
class SACAgent(Agent):
    ...
    def __init__(self, obs_dim, action_dim, action_range, device, critic_cfg,
                 ...
                 batch_size, learnable_temperature, need_encoder, encoder_cfg):
        super().__init__()
        ...
        if need_encoder:  # plugin an encoder
            encoder = make_encoders(encoder_cfg).to(self.device)
            # tie up the encoder of actor and critic
            self.critic.set_image_encoder(encoder)
            self.actor.set_image_encoder(encoder)
            self.critic_target.set_image_encoder(encoder)
